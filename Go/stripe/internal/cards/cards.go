...
func (c *Card) SubscribeToPlan(cost *stripe.Customer, plan, _, last4, cardType string) (*stripe.Subscription, error) {
	stripeCustomerID := cost.ID
...
func (c *Card) CreateCustomer(pm, email string) (*stripe.Customer, string, error) {
	...
	cost, err := customer.New(customerParams)
	if err != nil {
		msg := ""
		if stripeErr, ok := err.(*stripe.Error); ok {
			msg = cardErrorMessage(stripeErr.Code)
